...
final class PhutilCalendarRecurrenceRule extends PhutilCalendarRecurrenceSource
{
    ...
    protected function nextMinute()
    {
        ...
        while (!$this->setMinutes) {
          ...
          $by_minute);
            } elseif ($scale < self::SCALE_MINUTELY) {
                $minutes = $this->newMinutesSet(
          1,
          array());
            } else {
    ...
    private function isCursorBehind(array $cursor, array $state, $scale)
    {
        ...
        if ($cursor['month'] < $state['month']) {
            return true;
        } elseif ($cursor['month'] > $state['month']) {
            return false;
        }
