  ...
  private function computeRuleReducible($rule, array &$reducible)
  {
    ...
    foreach ($productions as $production) {
        ...
        if ($symbol == $end) {
            ...
            continue;
        } elseif (!$this->isRuleReducible($symbol, $reducible)) {
            $can_reduce = false;
            break;
        }
        }
        ...
        foreach ($this->states as $state => $items) {
            ...
            foreach ($items as $item) {
                ...
                if ($next == $end) {
                    if ($item[0] !== $init) {
                        $reduce[$item[3]][] = $item;
                    } elseif ($item[0] === $init && $item[3] === $eof) {
                        $accept = $item;
                    }
                } elseif ($this->isTerminal($next)) {
                    $shift[$next] = $item;
                } else {
